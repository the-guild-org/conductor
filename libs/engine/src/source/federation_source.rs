...
pub fn load_supergraph(
    ...
    match &config.supergraph {
        ...
        SupergraphSourceConfig::Remote {
            ...
            panic!("Remote supergraph source not supported in wasm32 at the moment, please fetch it from ENV");
        }
        #[cfg(not(target_arch = "wasm32"))]
        SupergraphSourceConfig::Remote {
            url,
        ...
        loop {
            ...
            match fetch_supergraph_schema(&url, headers.as_ref()) {
                Ok(new_schema) => {
                    ...
                    tracing::info!(
                        "Successfully updated supergraph schema after being fetched from {url}"
                    );
                }
    ...
    fn execute<'a>(
        ...
        Box::pin(wasm_polyfills::call_async(async move {
            ...
            match execute_federation(&supergraph, operation).await {
                Ok(response_data) => {
                    let mut response =
                        serde_json::from_str::<GraphQLResponse>(&response_data).unwrap();
