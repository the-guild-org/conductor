      ...
      return (
        ...
        Object.entries(definition.properties || {})
          .map(([propName, rawDef]) => {
            ...
            const definitionTypes = Array.isArray(fieldDef.type)
              ? fieldDef.type
              : [fieldDef.type];
            ...
            const defaultValue =
              (rawDef as JSONSchema7).default || fieldDef.default;
            ...
            if (
              ...
              return `<DocumentationContainer title={${JSON.stringify(
                propName
              )}} tags={["literal|${fieldDef.enum[0]}", "required"]}>
${makeDescription((rawDef as JSONSchema7).description || fieldDef.description)}
</DocumentationContainer>`;
            }

            return `<DocumentationContainer title={"${propName}"} tags={${JSON.stringify(
              tags
            )}}>
${makeDescription((rawDef as JSONSchema7).description || fieldDef.description)}
${ignoreList.includes(propName) ? "" : visitDefinition(root, fieldDef)}
      ...
      const itemDefinition = tryToResolveRef(
        root,
...
${makeDescription(
  (item as JSONSchema7).description || itemDefinition.description
)}
  ...
  const getStaticProps: GetStaticProps = async (ctx) => {
    ...
    if (
      ...
      items={${JSON.stringify(
        schema.examples.map(
          (v, i) =>
            (v as ExampleWithMetadata).$metadata?.title || `Example ${i + 1}`
        )
      )}}
    >
      ${schema.examples
