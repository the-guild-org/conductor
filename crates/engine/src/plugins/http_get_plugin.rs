use conductor_config::plugins::HttpGetPluginConfig;
...
use conductor_common::{
    ...
    http::{
        extract_accept, extract_content_type, parse_query_string, ConductorHttpRequest, Method,
        Mime, StatusCode, APPLICATION_JSON, APPLICATION_WWW_FORM_URLENCODED,
    },
...
impl Plugin for HttpGetPlugin {
    async fn on_downstream_http_request(&self, ctx: &mut RequestExecutionContext) {
        if ctx.downstream_http_request.method == Method::GET {
            let (_, accept, result) =
                extract_graphql_from_get_request(&ctx.downstream_http_request);
